...
class DriverLayout extends StatefulWidget {
  const DriverLayout({super.key, this.currentIndex = null});
...
class _DriverLayoutState extends State<DriverLayout> {
  ...
  @override
  void didUpdateWidget(covariant DriverLayout oldWidget) {
        ...
        return PopScope(
          ...
          child: Scaffold(
            ...
            floatingActionButton: FloatingActionButton(
              onPressed: () {
                // KYC gate: block trip creation if account is not yet activated
                final storage = di.sl<LocalStorage>();
                final isActive =
                    (storage.read(key: 'driver_kyc_active') as String?) == 'true';
                if (!isActive) {
            ...
            body: driverLayoutCubit.driverLayoutBottomScreens[currentIndexState],
            bottomNavigationBar: AnimatedBottomNavigationBar.builder(
              tabBuilder: (int index, bool isActive) {
                final icon = driverLayoutCubit.driverLayoutBottomIcons[index];
              ...
              itemCount: driverLayoutCubit.driverLayoutBottomIcons.length,
...
/// Shows a dialog explaining why trip creation is blocked and offers a
/// shortcut to the My Documents screen.
