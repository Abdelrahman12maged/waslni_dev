...
class _TripRatingViewState extends State<TripRatingView> {
  ...
  void _navigateHome() {
    ...
    if (context.canPop()) {
      ...
      context.go(widget.isDriverRatingPassenger
      ...
      child: BlocConsumer<RatingsCubit, RatingsState>(
        listener: (context, state) {
          if (state is RatingSubmitSuccess) {
            ...
            showToast(
              text:
                  widget.isEdit ? s.ratingUpdateSuccess : s.ratingSubmitSuccess,
              state: ToastStates.SUCESS,
            );
            _navigateHome();
          } else if (state is RatingSubmitError) {
            final errorMsg = RatingsCubit.mapErrorCode(
                context, state.errorCode, state.message);
            showToast(
              text: errorMsg,
