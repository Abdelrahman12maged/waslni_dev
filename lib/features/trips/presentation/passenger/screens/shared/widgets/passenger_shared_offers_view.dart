...
class PassengerSharedOffersView extends StatefulWidget {
  final Trip? trip;
  final int? id;
  final double? proposed_fare;
  final bool? is_auto_accept;

  const PassengerSharedOffersView({
    super.key,
    this.trip,
    this.id,
    this.proposed_fare,
  ...
  @override
  State<PassengerSharedOffersView> createState() =>
      _PassengerSharedOffersViewState();
}

class _PassengerSharedOffersViewState extends State<PassengerSharedOffersView>
  ...
  String get _resolvedDriverName => widget.trip?.driver?.name ?? '';
  String get _resolvedTripDatetime => widget.trip?.tripDatetime ?? '';
    ...
    if (newOffersList is List) {
      for (final item in newOffersList) {
        if (item is Offer) {
          ...
          try {
            parsedOffers.add(Offer.fromMap(item));
          } catch (_) {}
        } else if (item is Map) {
          try {
            parsedOffers.add(Offer.fromMap(Map<String, dynamic>.from(item)));
          } catch (_) {}
        }
      }
    }
  ...
  Future<void> _doAcceptOffer(Offer offer) async {
    ...
    final tripId =
        widget.trip?.id ?? widget.id ?? int.tryParse(_resolvedId) ?? 0;
    ...
    final updatedTrip = widget.trip?.copyWith(
        ...
        Trip(
          ...
          genderPreference:
              widget.trip?.genderPreference ?? GenderPreference.noPreference,
                    ...
                    Stack(
                      ...
                      children: [
                        CircleAvatar(
                          ...
                          backgroundImage:
                              appCachedImageProvider(photoUrl?.toString()),
                          child:
