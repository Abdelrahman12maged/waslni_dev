...
class _DriverProfileViewState extends State<DriverProfileView> {
  ...
  void _applyUserData(UserProfile userProfile) {
    ...
    setState(() {
      ...
      if (avatarUrl != null && avatarUrl!.isNotEmpty) {
        di.sl<LocalStorage>()
            .saveString(key: 'profile_picture_url', value: avatarUrl!);
        di.sl<LocalStorage>()
            .saveString(key: 'driver_image', value: avatarUrl!);
  ...
  Future<void> _pickAndUploadAvatar(ImageSource source) async {
    ...
    try {
      ...
      final userId =
          int.tryParse(rawUid?.toString() ?? '') ?? (user?.id ?? 0);
      ...
      await context
          .read<ProfileCubit>()
          .updateUserMyProfile(profileData: formData);
    } catch (e) {
                            ...
                            ListView(
                              ...
                              children: [
                                ...
                                if (userLangController.text.isNotEmpty) ...[
                                  const SizedBox(height: 16.0),
                                  defaultText(
                                      text: S.of(context).preferredLanguage),
                                  const SizedBox(height: 5.0),
                                  defaultFormField(
