...
class PassengerCurrentPrivateTripScreenClean extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<MapCubit>(
      ...
      child: _PassengerCurrentPrivateTripContent(
        trip: trip,
        tripId: tripId,
...
class _PassengerCurrentPrivateTripContent extends StatefulWidget {
  final Trip? trip;
  final int? tripId;
  final dynamic tripDetails;
  final dynamic userData;

  const _PassengerCurrentPrivateTripContent({
    this.trip,
    this.tripId,
    this.tripDetails,
    this.userData,
  ...
  @override
  State<_PassengerCurrentPrivateTripContent> createState() =>
      __PassengerCurrentPrivateTripContentState();
}

class __PassengerCurrentPrivateTripContentState
    extends State<_PassengerCurrentPrivateTripContent>
            ...
            if (mounted && loadedTrip != null) {
              setState(() {
                if (loadedTrip is Trip) {
                  ...
                  try {
                    _trip = TripModel.fromJson(Map<String, dynamic>.from(loadedTrip));
                  } catch (_) {}
                }
              });
              _initTripFlow();
  ...
  void _onDriverLocationUpdate(Map<String, dynamic>? data) {
    ...
    // ── 2. Full smooth marker/bearing animation (runs on EVERY GPS update) ──
  ...
  void _onArriveCar() {
    try {
      di.sl<PassengerAddPrivateTripCubit>().changePassengerStatus(
            in_car: 1,
  ...
  void _onChat() {
    final String driverName = _trip?.driver?.name.isNotEmpty == true
        ? _trip!.driver!.name
        : (tripDetails['driver']?['name']?.toString() ?? S.of(context).driverLabel);
    ...
    final membersList = [
      if (_trip?.driver != null)
        {'id': _trip!.driver!.id, 'name': _trip!.driver!.name, 'phone': _trip!.driver!.phone}
      else if (tripDetails['driver'] != null)
        tripDetails['driver'],
      if (_trip?.creator != null)
        {'id': _trip!.creator!.id, 'name': _trip!.creator!.name, 'phone': _trip!.creator!.phone}
    ...
    navigateTo(
      ...
      TripChatScreenClean(
        ...
        tripFrom: _trip?.fromLocationName ?? tripDetails['from_location_name']?.toString() ?? '',
        tripTo: _trip?.toLocationName ?? tripDetails['to_location_name']?.toString() ?? '',
        tripDatetime: _trip?.tripDatetime ?? tripDetails['trip_datetime']?.toString() ?? '',
        acceptedPrice: price,
        tripId: _trip?.id ?? int.tryParse(tripDetails['id']?.toString() ?? '') ?? 0,
        offerId:
            int.tryParse(tripDetails['accepted_offer_id']?.toString() ?? '0') ??
                0,
        driverId: _trip?.driverId ?? _trip?.driver?.id ?? int.tryParse(tripDetails['driver_id']?.toString() ?? '') ?? 0,
        tripType: 'private',
        members: membersList,
      ),
    );
  ...
  void _onChangeDriver() {
    final tId = _trip?.id ?? widget.tripId ?? int.tryParse(tripDetails['id']?.toString() ?? '') ?? 0;
    final offId = _trip?.driverId ?? int.tryParse(tripDetails['accepted_offer_id']?.toString() ?? '');
    _confirmAction(
      ...
      onConfirmed: () async {
        final ok = await sl<PassengerTripsCubit>().cancelAcceptedOfferAndReopen(
          tripId: tId,
          offerId: offId,
          trip: _trip,
        );
      ...
      builder: (context, state) {
        return AnnotatedRegion<SystemUiOverlayStyle>(
          ...
          child: Scaffold(
            ...
            body: Stack(
              children: [
                // ── 1. Fullscreen Map (Map Feature Clean Widget) ────────
                Positioned.fill(
                  child: BlocProvider<MapCubit>(
                    create: (_) => sl<MapCubit>(),
                    child: AppMapWidget(
                      mode: AppMapMode.view,
                      initialPosition: PassengerCurrentPrivateTripScreenClean
                          ._kDefaultCamera.target,
                      markers: _markers,
