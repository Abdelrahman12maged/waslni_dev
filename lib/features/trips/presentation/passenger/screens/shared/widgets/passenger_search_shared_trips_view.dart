...
class PassengerSearchSharedTripsView extends StatelessWidget {
  const PassengerSearchSharedTripsView({super.key});
            ...
            if (isJoined)
              Container(
                ...
                decoration: const BoxDecoration(
                  ...
                  border: Border(
                    bottom:
                        BorderSide(color: Color(0xFF86EFAC), width: 1),
                  ),
                ),
                child: Row(
                  children: [
  ...
  int get _currentUserId {
    ...
    try {
      if (Hive.isBoxOpen('hive_box')) {
        ...
        if (rawUser is Map) {
          final uId =
              rawUser['id'] ?? rawUser['user']?['id'] ?? rawUser['data']?['id'];
