...
import 'package:car_app/features/settings/presentation/driver/screens/driver_profile_screen.dart';
...
class DriverSettingsTabScreen extends StatefulWidget {
  const DriverSettingsTabScreen({super.key});

  @override
  State<DriverSettingsTabScreen> createState() =>
      _DriverSettingsTabScreenState();
}

class _DriverSettingsTabScreenState extends State<DriverSettingsTabScreen> {
  bool isEnglish = true;
  ...
  Future<void> _performLogout(BuildContext context) async {
    ...
    try {
      final rememberMe = _localStorage.read(key: 'remember_me') as bool? ?? false;
      final savedMobile = _localStorage.read(key: 'saved_mobile') as String?;
      final savedPassword = _localStorage.read(key: 'saved_password') as String?;
      final lang = _localStorage.read(key: 'lang') as String?;

      await _localStorage.clear();

      // Ensure onboarding is marked completed so logout always redirects to Login, never Onboarding
      ...
      if (rememberMe && savedMobile != null && savedPassword != null) {
        ...
        await _localStorage.saveString(key: 'saved_password', value: savedPassword);
      }
    } catch (e) {
      ...
      child: Column(
        children: [
          settingsTile(
            ...
            onTap: () {
              navigateTo(context, const DriverProfileScreen());
            },
          ),
          ...
          settingsTile(
            ...
            leadingIconColor: AppColors.accent,
            titleText: S.of(context).myDocuments,
            trailingIcon: Icons.keyboard_arrow_right_outlined,
            tileColor: Colors.white,
            onTap: () {
          ...
          settingsTile(
            ...
            leadingIconColor: Colors.amber.shade700,
            titleText: S.of(context).driverRatingsTitle,
            trailingIcon: Icons.keyboard_arrow_right_outlined,
            tileColor: Colors.white,
            onTap: () {
              final driverId = int.tryParse(
                    _localStorage.read(key: 'userid')?.toString() ??
                    _localStorage.read(key: 'user_id')?.toString() ??
                    _localStorage.read(key: 'driver_id')?.toString() ??
                    '',
                  ) ??
                  0;
              context.push(AppRoutes.driverRatings, extra: {'driver_id': driverId});
            },
          ),
                  ...
                  children: [
                    ...
                    GestureDetector(
                      ...
                      child: ListTile(
                        ...
                        title: defaultText(
                          text: S.of(context).userlayoutsettingsLanguageenglish,
                          textAlign: TextAlign.center,
                        ),
                        tileColor: !isEnglish ? Colors.white : AppColors.accent,
                      ),
                    ),
                    const SizedBox(height: 10.0),
                    GestureDetector(
                              ...
                              children: [
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    border: Border.all(
                                      width: 1,
                                      color: isLoggingOut ? Colors.grey.shade300 : Colors.black,
                                    ),
                                  ),
                                  child: MaterialButton(
                                    ...
                                    child: Text(
                                      S.of(context).no,
                                      style: TextStyle(
                                        color: isLoggingOut ? Colors.grey : Colors.black,
                                        fontSize: 16.0,
