...
class _PassengerHomeViewState extends State<PassengerHomeView> {
  ...
  void initState() {
    super.initState();
    TripSecurityService.syncActiveTripWithServer(
        sl<ApiClient>(), sl<LocalStorage>());
      ...
      child: BlocBuilder<PassengerHomeCubit, PassengerHomeState>(
        builder: (context, state) {
          return RefreshIndicator(
            onRefresh: () async {
              await TripSecurityService.syncActiveTripWithServer(
                  sl<ApiClient>(), sl<LocalStorage>());
              if (mounted) setState(() {});
            },
                  ...
                  Row(
                    children: [
                      tripCard(
                        ...
                        onTap: () =>
                            context.push(AppRoutes.passengerAddPrivateTrip),
                      ),
  ...
  Widget _buildActivePreTripBanner(BuildContext context) {
    ...
    final fromName = activeTrip.fromLocationName.isNotEmpty
        ? activeTrip.fromLocationName
        : S.of(context).pickupPoint;
    final toName = activeTrip.toLocationName.isNotEmpty
        ? activeTrip.toLocationName
        : S.of(context).destinationPoint;
    final driverName = activeTrip.driver?.name.isNotEmpty == true
