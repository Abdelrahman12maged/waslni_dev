...
class DriverDocumentsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<DriverDocumentsCubit>(
      ...
      child: const _DriverDocumentsView(),
    );
  }
}

class _DriverDocumentsView extends StatelessWidget {
  const _DriverDocumentsView();
          ...
          Row(
            ...
            children: [
              Icon(
                isActive
                    ? Icons.verified_user
                    : (hasMissing ? Icons.warning_amber_rounded : Icons.hourglass_top_rounded),
                color: isActive
                    ? Colors.green.shade600
                    : (hasMissing ? Colors.orange.shade800 : Colors.amber.shade800),
                size: 22,
              ),
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        ...
                        color: isActive
                            ...
                            : (hasMissing ? Colors.orange.shade900 : Colors.amber.shade900),
                        fontSize: 14,
                      ),
                    ),
...
class _DocumentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<DriverDocumentsCubit, DriverDocumentsState>(
      ...
      builder: (ctx, state) {
        // Determine local pending file for this type
        File? pendingFile;
        switch (docType) {
          case 'national_id':
        ...
        return Card(
          ...
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(12),
            side: BorderSide(
              color: _borderColor(isUploaded, isExpired, pendingFile != null, isLocked),
              width: 1.5,
            ),
          ),
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // ── Action: Locked Notice OR Pick/Replace button ──────────────
                if (isLocked)
                  Container(
                    width: double.infinity,
                    padding:
  ...
  Color _borderColor(bool isUploaded, bool isExpired, bool hasPending, bool isLocked) {
...
class _StatusChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (isExpired) {
      return _chip(context, S.of(context).documentExpired, Colors.red.shade700,
          Colors.red.shade50, Icons.warning_amber_outlined);
    }
    if (isLocked) {
      return _chip(context, S.of(context).documentApprovedLocked,
          Colors.green.shade800, Colors.green.shade100, Icons.lock_outline);
    }
    if (isUploaded) {
      return _chip(context, S.of(context).documentUploaded,
          Colors.green.shade700, Colors.green.shade50, Icons.check_circle_outline);
    }
