...
// ─────────────────────────────────────────────────────────────────────────────
class PrivateCurrentScreenCleanoffers extends StatefulWidget {
  ...
  @override
  State<PrivateCurrentScreenCleanoffers> createState() =>
      _PrivateCurrentScreenCleanoffersState();
}

class _PrivateCurrentScreenCleanoffersState
    extends State<PrivateCurrentScreenCleanoffers>
  ...
  // Track rejected offer IDs to prevent them from EVER reappearing
  ...
  void initState() {
    ...
    if (_trip == null && numericTripId > 0) {
      try {
        final trips = PassengerTripsCubit.get(context).TripsListByTypeCurrentPrivete;
  ...
  void _initMapElements() {
    ...
    _markers = {
      Marker(
        ...
        infoWindow: InfoWindow(title: _resolvedFromLocation.isNotEmpty ? _resolvedFromLocation : S.current.originPoint),
      ),
      Marker(
        markerId: const MarkerId('dest_point'),
        position: destLatLng,
        icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueRed),
        infoWindow: InfoWindow(title: _resolvedToLocation.isNotEmpty ? _resolvedToLocation : S.current.destinationPoint),
      ),
    };
  ...
  Future<void> _fetchRoutePolyline(LatLng from, LatLng to) async {
    try {
      ...
      log('Error fetching road polyline: $e', name: 'PrivateCurrentScreenCleanoffers');
    }
  }
  ...
  // ── Formatting Helpers ────────────────────────────────
  // (see top-level _formatPrice below)
  ...
  void _stopLoading(dynamic newOffersList) {
    ...
    for (final item in rawList) {
      if (item is Offer) {
        ...
        try {
          parsed.add(OfferModel.fromJson(Map<String, dynamic>.from(item)));
        } catch (_) {}
      }
    }
  ...
  void _acceptOffer(Offer offer) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          S.of(context).confirmOfferQuestion(offer.price.toStringAsFixed(2), S.of(context).jod),
  ...
  Future<void> _doAcceptOffer(Offer offer) async {
    ...
    final tripId = _trip?.id ?? widget.trip?.id ?? widget.id ?? int.tryParse(_resolvedId) ?? 0;
    ...
    if (mounted) {
      navigateTo(
        ...
        TripChatScreenClean(
          ...
          tripId: tripId,
          offerId: offer.id,
          driverId: offer.driverId > 0 ? offer.driverId : (offer.driver?.id ?? 0),
          isOffersPhase: true,
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              showToast(text: S.of(context).orderCanceledSuccessfully, state: ToastStates.SUCESS);
    ...
    return PopScope(
      ...
      body: Stack(
        children: [
          ...
          // ── 4. White Bottom Sheet (Cleaned: No Timer, No Auto-Accept, No Fare Stepper) ──
...
// ─────────────────────────────────────────────────────────────────────────────
//  Driver Bidding Offer Card (Compact & Clean - No Auto-Dismiss Timer)
// ─────────────────────────────────────────────────────────────────────────────
...
class _DriverBiddingCard extends StatelessWidget {
  final Offer offer;
  final Trip? trip;
  final int index;
  ...
  Widget build(BuildContext context) {
    ...
    final ratingStr = driver?.rating?.isNotEmpty == true
        ? driver!.rating!
        : (driver?.ratingAvg != null ? driver!.ratingAvg!.toStringAsFixed(1) : null);
    ...
    final carModel = carParts.isNotEmpty
        ? carParts.join(' ')
        : (car?.plateNumber.isNotEmpty == true ? PlateNumberFormatter.format(car!.plateNumber) : S.of(context).driverCarLabel);
                  ...
                  children: [
                    CircleAvatar(
                      ...
                      child: (driver?.photo == null ||
                              ApiEndpoints.buildImageUrl(driver!.photo) == null)
