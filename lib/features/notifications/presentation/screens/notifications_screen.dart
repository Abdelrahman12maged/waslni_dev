...
class CleanNotificationsScreen extends StatefulWidget {
  const CleanNotificationsScreen({super.key});

  @override
  State<CleanNotificationsScreen> createState() => _CleanNotificationsScreenState();
}

class _CleanNotificationsScreenState extends State<CleanNotificationsScreen> {
  bool _isLoading = true;
  final Set<int> _readNotificationIds = {};
  int _selectedFilterIndex = 0; // 0: All, 1: Unread, 2: Offers, 3: Trips
  ...
  bool _isNotificationRead(AppNotification notification) {
    ...
    if (notification.id != null && _readNotificationIds.contains(notification.id)) {
      return true;
    }
    return false;
  }
  ...
  String _formatNotificationTime(String? rawDate, bool isArabic) {
    ...
    try {
      ...
      if (difference.inSeconds < 60) {
        ...
        final timeStr = DateFormat('hh:mm a', isArabic ? 'ar' : 'en').format(dateTime);
        return S.current.todayAt(timeStr);
      } else if (difference.inDays < 2 || (difference.inHours < 48 && now.day - dateTime.day == 1)) {
        final timeStr = DateFormat('hh:mm a', isArabic ? 'ar' : 'en').format(dateTime);
  ...
  _NotificationCategory _categorizeNotification(String title, String description) {
    ...
    if (combined.contains('عرض') ||
      ...
      return _NotificationCategory.offer;
    }
    if (combined.contains('بدأت') ||
        combined.contains('start') ||
        combined.contains('وصل') ||
        combined.contains('arrived') ||
        combined.contains('في الطريق') ||
        combined.contains('on the way')) {
      return _NotificationCategory.tripStarted;
    }
    if (combined.contains('اكتملت') ||
        combined.contains('إنهاء') ||
        combined.contains('completed') ||
        combined.contains('finished')) {
      return _NotificationCategory.tripCompleted;
    }
    if (combined.contains('إلغاء') ||
        combined.contains('ملغاة') ||
        combined.contains('cancel')) {
      return _NotificationCategory.tripCanceled;
    }
    if (combined.contains('رسالة') ||
        combined.contains('دردشة') ||
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<NotificationsCubit, NotificationsState>(
      ...
      builder: (context, state) {
        ...
        final unreadCount = allNotifications.where((n) => !_isNotificationRead(n)).length;

        // Apply tab filter
        final filteredList = allNotifications.where((n) {
          final isRead = _isNotificationRead(n);
          final title = (isArabic ? n.titleAr : n.titleEn) ?? n.titleAr ?? n.titleEn ?? '';
          final desc = (isArabic ? n.descriptionAr : n.descriptionEn) ?? n.descriptionAr ?? n.descriptionEn ?? '';
          final category = _categorizeNotification(title, desc);

          if (_selectedFilterIndex == 1) {
            return !isRead; // Unread only
          } else if (_selectedFilterIndex == 2) {
            return category == _NotificationCategory.offer; // Offers
          } else if (_selectedFilterIndex == 3) {
            return category == _NotificationCategory.tripStarted ||
                category == _NotificationCategory.tripCompleted ||
                category == _NotificationCategory.tripCanceled; // Trips
          }
          return true; // All
        }).toList();
              ...
              if (!_isLoading && allNotifications.isNotEmpty)
                Container(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Row(
                      children: [
                        _buildFilterChip(index: 0, label: '${S.of(context).allNotificationsFilter} (${allNotifications.length})'),
                        const SizedBox(width: 8),
                        _buildFilterChip(index: 1, label: '${S.of(context).unreadNotificationsFilter} ($unreadCount)'),
                        const SizedBox(width: 8),
                        _buildFilterChip(index: 2, label: S.of(context).offersFilter),
                        const SizedBox(width: 8),
                        _buildFilterChip(index: 3, label: S.of(context).tripsFilter),
                      ],
                    ),
                  ),
                ),
                ...
                child: _isLoading
                    ...
                    : RefreshIndicator(
                        ...
                        child: filteredList.isEmpty
                            ? _buildEmptyState(context, _selectedFilterIndex > 0)
                            : ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final category = _categorizeNotification(title, description);
                                  ...
                                  return _NotificationCard(
                                    ...
                                    onTap: () {
                                      ...
                                      // 1. Resolve tripId accurately (from entity, data payload, or explicit #ID)
                                      int? effectiveTripId = notification.tripId;
                                      if (effectiveTripId == null || effectiveTripId <= 0) {
                                        if (notification.data != null) {
                                          effectiveTripId = int.tryParse(notification.data!['trip_id']?.toString() ?? '') ??
                                              int.tryParse(notification.data!['tripId']?.toString() ?? '') ??
                                              int.tryParse(notification.data!['id']?.toString() ?? '');
                                        }
                                      }
                                      if (effectiveTripId == null || effectiveTripId <= 0) {
                                        final match = RegExp(r'#(\d+)').firstMatch('$title $description');
                                        if (match != null) {
                                          effectiveTripId = int.tryParse(match.group(1) ?? '');
                                        }
                                      }

                                      // 2. Deduced type
                                      String deducedType = notification.type ?? 'general';
                                      switch (category) {
                                        case _NotificationCategory.offer:
                                          deducedType = 'new_offer';
                                          break;
                                        case _NotificationCategory.tripStarted:
                                          deducedType = 'trip_started';
                                          break;
                                        case _NotificationCategory.tripCompleted:
                                          deducedType = 'trip_completed';
                                          break;
                                        case _NotificationCategory.tripCanceled:
                                      ...
                                      // 3. For general notifications or notifications without an active trip, show details dialog instead of redundant routing
                                      if (category == _NotificationCategory.general && (effectiveTripId == null || effectiveTripId <= 0)) {
                                        _showNotificationDetailsDialog(context, title, description, timeFormatted);
                                        return;
                                      }

                                      // 4. Delegate to NotificationRoutingService
                                      di.sl<NotificationRoutingService>().handleNotificationTap({
                                        'type': deducedType,
        ...
        Center(
          child: Column(
            ...
            children: [
              ...
              Text(
                isFiltered ? S.of(context).noMatchingNotifications : S.of(context).noNotificationsYet,
                style: GoogleFonts.cairo(
                  fontWeight: FontWeight.bold,
                  fontSize: 17,
                  color: const Color(0xFF1E293B),
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(18),
          border: Border.all(
            color: isRead ? const Color(0xFFE2E8F0) : AppColors.primary.withOpacity(0.35),
            width: isRead ? 1 : 1.5,
          ),
          boxShadow: [
            BoxShadow(
              color: isRead ? Colors.black.withOpacity(0.02) : AppColors.primary.withOpacity(0.06),
              blurRadius: 10,
                ...
                children: [
                  Row(
                    children: [
                      Expanded(
                        child: Text(
                          title.isNotEmpty ? title : S.of(context).newNotificationTitle,
                          style: GoogleFonts.cairo(
                            fontSize: 14,
